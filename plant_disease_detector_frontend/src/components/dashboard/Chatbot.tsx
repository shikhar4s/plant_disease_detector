...
export default function Chatbot() {
  ...
  const { selectedImage } = usePlantData();
  ...
  return <div className="fixed bottom-4 right-4 z-40">
    {isOpen && <section aria-label={t('chatbot.title')} className="bg-white rounded-2xl shadow-2xl border w-80 max-w-[calc(100vw-2rem)] h-[28rem] max-h-[75vh] mb-3 flex flex-col">
      <div className="bg-gradient-to-r from-green-500 to-emerald-600 text-white p-4 rounded-t-2xl flex justify-between items-center">
        ...
        <button onClick={() => setIsOpen(false)} aria-label={t('features.close')} className="p-1"><XMarkIcon className="w-5 h-5" /></button>
      </div>
    ...
    <button onClick={() => setIsOpen(value => !value)} aria-expanded={isOpen} aria-label={t('features.openChat')}
