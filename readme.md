...
- India-only, state-first city/town selection or user-triggered GPS location, with Open-Meteo current conditions and a seven-day forecast; an attributed MET Norway forecast is used if Open-Meteo throttles the forecast request.
...
The Weather page loads Indian states and union territories first, then the available cities and towns for the selected state; no name needs to be typed. Its bundled [GeoNames cities500](https://download.geonames.org/export/dump/cities500.zip) snapshot has 6,986 Indian places across 36 states/territories as of 18 September 2026 under CC BY 4.0. Smaller settlements and some names may be absent or outdated; this is not a definitive list of every Indian city. Run `scripts/generate_india_weather_catalog.py` to refresh the snapshot. "Use my location" requests browser permission only when clicked. The backend first checks the submitted coordinates against a bundled [GeoNames India boundary](https://download.geonames.org/export/dump/shapes_simplified_low.json.zip); uncertain coastal/border points receive a rate-limited, cached [OpenStreetMap Nominatim reverse country check](https://operations.osmfoundation.org/policies/nominatim/). If India cannot be confirmed, no forecast is returned and the user can choose a state and city. This is a practical geofence, not an authoritative border survey. Run `scripts/generate_india_weather_boundary.py` to refresh the boundary.
